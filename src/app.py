...
for i in range(EPOCHS):
    model.fit([anchors, positives, negatives], dummy_y, batch_size=BATCH_SIZE, epochs=5)
    anchors, positives, negatives = random_triplets(x_train, y_train, TRAIN_TRIPLES)
    print(compute_pseudo_accuracy(model, valid_triplets))

emb_model = embedding_model(INPUT_SHAPE, EMBEDDING_SIZE)
emb_model.set_weights(model.get_weights())

print(emb_model.predict(x_test))
print(y_test)
